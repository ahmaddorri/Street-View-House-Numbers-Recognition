...
w_conv2=tf.Variable(tf.truncated_normal([7,7,16,512], stddev=0.1))
...
h_pool_final_drop = tf.nn.dropout(h_pool_final, keep_prob)


#Densely Connected Layer

num_hidden1=20
W_fc1 = tf.Variable(tf.truncated_normal([8*8*(16+512), num_hidden1], stddev=0.1))
b_fc1 = tf.Variable(tf.constant(0.1, shape=[num_hidden1]))


h_final_flat = tf.reshape(h_pool_final_drop, [-1, 8*8*(16+512)])
h_fc1 = tf.nn.relu(tf.matmul(h_final_flat, W_fc1) + b_fc1)


#Readout layer

W_fc2 = tf.Variable(tf.truncated_normal([num_hidden1, 11], stddev=0.1))
