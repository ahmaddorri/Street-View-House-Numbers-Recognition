...
w_conv2=tf.Variable(tf.truncated_normal([7,7,16,32], stddev=0.1))
...
w_conv3=tf.Variable(tf.truncated_normal([5,5,48,64], stddev=0.1))
conv3 = tf.nn.conv2d(input=h_pool_final,filter=w_conv3,strides=[1,1,1,1],padding="SAME")
...
W_fc1 = tf.Variable(tf.truncated_normal([8*8*64, 100], stddev=0.1))
...
#Readout layer
W_fc2 = tf.Variable(tf.truncated_normal([100, 11], stddev=0.1))
